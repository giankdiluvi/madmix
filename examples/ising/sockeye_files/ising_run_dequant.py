...
sys.path.insert(1, '../../../src/')
import concrete
from dequantization import train_dequant_discrete
...
#########################
#########################
parser = argparse.ArgumentParser(description="Fit a RealNVP normalizing flow to an Ising model with dequantized nodes")
...
########################
########################
print('Training a RealNVP normalizing flow for an Ising model with dequantized nodes')
print()
print('M: '+M)
print()
print('Flow settings:')
...
tmp_flow,tmp_loss=train_dequant_discrete(
    depth=depth,sample=sample,width=width,max_iters=max_iters,lr=lr
)
...
pkl_save(tmp_flow, outpath+'cache/0'+str(idx)+'_'+filename+'_flows_dequant')
pkl_save(tmp_loss, outpath+'cache/0'+str(idx)+'_'+filename+'_losses_dequant')
print()

print('Saving results')
# load files
flows     = pkl_load(outpath+filename+'_flows_dequant')
losses    = pkl_load(outpath+filename+'_losses_dequant')
cpu_times = pkl_load(outpath+filename+'_cpu_times_dequant')
...
# save updated files
pkl_save(flows, outpath+filename+'_flows_dequant')
pkl_save(losses, outpath+filename+'_losses_dequant')
pkl_save(cpu_times, outpath+filename+'_cpu_times_dequant')
print('Done!')
