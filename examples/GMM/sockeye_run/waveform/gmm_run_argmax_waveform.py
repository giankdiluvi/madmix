...
#########################
#########################
parser = argparse.ArgumentParser(description="Fit a RealNVP normalizing flow with argmaxed label indices to a GMM with waveform data set")
...
parser.add_argument('--max_iters', type = float, default = 10001,
...
########################
########################
print('Training a RealNVP normalizing flow with argmaxed label indices for a GMM with the waveform data set')
print()
...
training_sample = amf.argmax_gmmsample_gen(pred_x,pred_w,pred_mu,pred_sigma)
...
# load files
flows     = pkl_load(outpath+'gmm_flows_argmax')
losses    = pkl_load(outpath+'gmm_losses_argmax')
cpu_times = pkl_load(outpath+'gmm_cpu_times_argmax')
...
# save updated files
pkl_save(flows, outpath+'gmm_flows_argmax')
pkl_save(losses, outpath+'gmm_losses_argmax')
pkl_save(cpu_times, outpath+'gmm_cpu_times_argmax')
print('Done!')
